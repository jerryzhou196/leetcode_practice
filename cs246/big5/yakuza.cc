...
ostream &operator<<(
    ostream &out,
    const Yakuza &y); // Should output name and title of y and all subordinatres
...
void Yakuza::setTitle(string newTitle) { title = newTitle; }
...
int main() {
  ...
  Yakuza moveMajima =
      std::move(majimaShadowClone); // Function to force move constructor
